      ...
      return (
        ...
        >
          <div className="text-white">
            <h2 className="text-2xl font-bold mb-4 text-center text-blue-300">Game Rules</h2> {/* Centered and colored title */}
            <div className="mb-4">
              <h3 className="text-lg font-semibold mb-2 text-blue-200">Objective:</h3> {/* Colored subheading */}
              <p>
                The goal is to collect as many smiles as possible by clicking the button as close to a whole second as you can.
              </p>
            </div>
            <div className="mb-4">
              <h3 className="text-lg font-semibold mb-2 text-blue-200">How to Play:</h3> {/* Colored subheading */}
            ...
            <div className="mb-4">
              <h3 className="text-lg font-semibold mb-2 text-blue-200">Earning Smiles:</h3> {/* Colored subheading */}
              <p className="mb-2">
              ...
              <table className="w-full border-collapse border border-gray-600">
                ...
                <tbody>
                  {smileRanges.map((range, index) => (
                    <tr key={index} className={index % 2 === 0 ? "bg-gray-700" : "bg-gray-800"}> {/* Alternating row colors */}
                      <td className="border border-gray-600 px-4 py-2 text-center"> {/* Centered text */}
                        {range.min}
                        {/* Display range correctly: 0, 1-10, 11-50, 51-100, 101+ */}
                        {range.max !== null && range.min !== range.max ? ` – ${range.max}` : ''}
                        {range.max === null && range.min !== 0 ? `${range.min}+` : ''}
                        {range.max !== null && range.min === range.max && range.min === 0 ? '' : ''} {/* Handle 0 case */}
                         {range.max === null && range.min === 0 ? '0+' : ''} {/* Handle 0+ case if needed, though unlikely */}

                      </td>
                      <td className="border border-gray-600 px-4 py-2 text-center">{range.smiles}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="mt-6 pt-4 border-t border-gray-700">
               <h3 className="text-lg font-semibold mb-2 text-blue-200">About the Creator:</h3> {/* Colored subheading */}

...
const Game: React.FC = () => {
  ...
  const handleAttempt = async () => {
    ...
    setTimeout(() => setIsButtonDisabled(false), 500) // Блокируем кнопку на 500 мс
    ...
    if (success) {
      ...
      if (updatedUser) {
        ...
        if (updatedUser.attempts_left <= 0) {
          toast.info(`Попытки закончились! Ваша скидка: ${formatDiscount(updatedUser.discount)}`)
          findBestResult(userAttempts)
        }
      }
    }
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4">
      ...
      <div className="max-w-md mx-auto bg-gray-800 rounded-lg shadow-lg p-6">
        ...
        <button
          ...
          disabled={isButtonDisabled || currentUser.attempts_left <= 0}
        >
          Нажать
        </button>
        ...
        <div className="mt-6">
          ...
          {currentUser.best_result !== null && (
            <p className="mb-2">Лучший результат: {currentUser.best_result} мс (Скидка: {formatDiscount(currentUser.discount)})</p>
          )}
          ...
          <table className="w-full mt-4 border-collapse">
            ...
            <tbody>
              {displayedAttempts.map((attempt, index) => (
                <tr 
                  key={index} 
                  className={currentUser.attempts_left <= 0 && index === bestResultIndex 
                    ? "bg-green-700" 
                    : index % 2 === 0 
                      ? "bg-gray-800" 
                      : "bg-gray-700"
              ...
              {displayedAttempts.length === 0 && (
                <tr>
                  <td colSpan={2} className="p-2 text-center">Нет попыток</td>
                </tr>
              )}
            </tbody>
          </table>

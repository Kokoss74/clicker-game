import { create } from 'zustand';
    ...
    export const useGameStore = create<GameState>((set, get) => ({
      ...
      loadSettings: async () => {
        try {
          ...
          const loadedSettings = settings ? {
            ...settings,
            smile_ranges: Array.isArray(settings.smile_ranges) ? settings.smile_ranges : defaultSmileRanges
          } : null;
      ...
      getSmileRanges: (): SmileRange[] => {
        ...
        if (settings && Array.isArray(settings.smile_ranges) && settings.smile_ranges.length > 0) {
